//! income-calc-core: pure income / US tax calculation engine (tax year 2025).
//!
...
pub use calc::{calculate, project};
pub use types::*;
...
mod tests {
    ...
    #[test]
    fn single_100k_federal_tax() {
        let out = calculate(&salary_input(100_000.0));
        // taxable = 100k - 15k = 85k
        assert!((out.federal_taxable_income - 85_000.0).abs() < 0.01);
        // 10%*11925 + 12%*(48475-11925) + 22%*(85000-48475) = 1192.5 + 4386 + 8035.5 = 13614
        assert!(
            (out.federal_tax - 13_614.0).abs() < 1.0,
            "got {}",
            out.federal_tax
        );
        // FICA: 6.2% + 1.45% of 100k = 7650
        assert!((out.fica.total - 7_650.0).abs() < 0.01);
        assert!((out.net_annual - (100_000.0 - 13_614.0 - 7_650.0)).abs() < 1.0);
    ...
    fn pretax_401k_reduces_income_tax_not_fica() {
        ...
        assert!((out.federal_taxable_income - 75_000.0).abs() < 0.01);
    ...
    fn child_tax_credit() {
        ...
        let out = calculate(&input);
        assert!((out.child_tax_credit - 4_000.0).abs() < 0.01);
    }
    ...
    fn state_flat_tax() {
        ...
        input.state = "IL".into();
        let out = calculate(&input);
        // 4.95% of 85k federal taxable
        assert!((out.state_tax.tax - 85_000.0 * 0.0495).abs() < 0.01);
    ...
    fn ltcg_stacking() {
        ...
        let out = calculate(&input);
        // Ordinary taxable = 35k; gains stack from 35k to 55k, 0% bracket ends at 48350.
        // 0% on 13,350; 15% on 6,650 = 997.5
        assert!(
            (out.capital_gains_tax - 997.5).abs() < 1.0,
            "got {}",
